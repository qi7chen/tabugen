...
public class GlobalPropertyDefine
{
    ...
    public void parseFrom(List<CSVRecord> records)
    {
        ...
        if (!records.get(9).get(3).isEmpty()) {
            this.EnableSearch = Boolean.parseBoolean(records.get(9).get(3));
        }
        {
            String[] kvList = records.get(10).get(3).split(AutogenConfigManager.TAKSI_ARRAY_DELIM);
        ...
        {
            String[] kvList = records.get(11).get(3).split(AutogenConfigManager.TAKSI_MAP_DELIM1);
            for(int i = 0; i < kvList.length; i++) {
