...
int GlobalPropertyDefine::ParseFromRows(const vector<CSVRow>& rows, GlobalPropertyDefine* ptr)
{
    ...
    {
        const std::vector<std::string>& array = absl::StrSplit(rows[10][2], TABULAR_ARRAY_DELIM);
    ...
    {
        const std::vector<std::string>& vec = absl::StrSplit(rows[11][2], TABULAR_MAP_DELIM1);
        for (size_t i = 0; i < vec.size(); i++)
        {
            const std::vector<std::string>& kv = absl::StrSplit(vec[i], TABULAR_MAP_DELIM2);
            ASSERT(kv.size() == 2);
            if(kv.size() == 2)
            {
