...
class CppCsvLoadGenerator:
    ...
    def gen_field_array_assign_stmt(self, prefix, typename, name, row_name, tabs):
        ...
        content = '%s{\n' % (self.TAB_SPACE * tabs)
        content += '%sconst std::vector<std::string>& array = absl::StrSplit(%s, TABULAR_ARRAY_DELIM);\n' % (space, row_name)
    ...
    def gen_field_map_assgin_stmt(self, prefix, typename, name, row_name, tabs):
        ...
        space = self.TAB_SPACE * (tabs + 1)
        content = '%s{\n' % (self.TAB_SPACE * tabs)
        content += '%sconst std::vector<std::string>& vec = absl::StrSplit(%s, TABULAR_MAP_DELIM1);\n' % (space, row_name)
        content += '%sfor (size_t i = 0; i < vec.size(); i++)\n' % space
        content += '%s{\n' % space
        content += '%s    const std::vector<std::string>& kv = absl::StrSplit(vec[i], TABULAR_MAP_DELIM2);\n' % space
